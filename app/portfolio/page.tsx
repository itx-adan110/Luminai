...
export default function PortfolioPage() {
  return <main className="section-pad tool-page">
    <div className={`tool-title ${styles.portfolioIntro}`}>
    ...
    <section className={styles.portfolioOverview} aria-label="Portfolio overview">
    ...
    <section className={styles.portfolioGrid} aria-label="Portfolio sections">
      {sections.map(([title, description], index) => <article className={styles.portfolioItem} key={title}>
        <span className={styles.portfolioIndex}>{String(index + 1).padStart(2, "0")}</span>
        <h2>{title}</h2>
        <p>{description}</p>
